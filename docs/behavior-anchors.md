...
- `pending-anchor` — capability declared, anchor test does not yet
  exist. Listed in `yole-challenges/baselines/bluff-baseline.txt` Section 3.
...
| CAP-026 | app | Connect to SMB share (fails when unreachable) | shared/src/commonTest/kotlin/digital/vasic/yole/network/protocols/smb/SmbServiceTest.kt::testConnectFailsWithInvalidHost | SmbService.connect() honestly reports reachability via real protocol negotiation (CONST-035) | active |
...
| CAP-033 | app | UI: light theme TextPrimary on SurfacePrimary meets WCAG AA contrast | shared/src/commonTest/kotlin/digital/vasic/yole/ui/ThemeWcagContrastTest.kt::light theme TextPrimary on SurfacePrimary meets WCAG AA | computed contrast ratio between editor.foreground and editor.background in the light theme palette (LegacyThemeBridge byte-equal to Yole-Light.json) is ≥ 4.5 | active |
