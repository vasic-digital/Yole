...
**Last updated:** 2026-05-15 (iter-60 F3 **Phase 2 COMPLETE** — VsCodeSnippetParser + SnippetRegistry + markdown anchor. Commit: `f0357c51` on master.

Phase 2 delivered:
- `shared/src/commonMain/kotlin/digital/vasic/yole/completion/snippet/Snippet.kt` — data class `Snippet(prefix, body, description?, scope)`. Was pre-created (untracked); verified matches plan spec verbatim.
- `shared/src/commonMain/kotlin/digital/vasic/yole/completion/snippet/SnippetParseException.kt` — RuntimeException subclass.
- `shared/src/commonMain/kotlin/digital/vasic/yole/completion/snippet/VsCodeSnippetParser.kt` — stateless `object VsCodeSnippetParser { fun parse(input: String): List<Snippet> }`. Uses kotlinx-serialization-json. Handles string-body and array-body forms; skips entries without `prefix`; throws SnippetParseException on malformed JSON.
- `shared/src/commonMain/kotlin/digital/vasic/yole/completion/snippet/SnippetRegistry.kt` — `object SnippetRegistry { fun forLanguage(langId): List<Snippet>; fun clear() }` backed by process-local cache. Uses `expect fun readSnippetResource(path: String): String?` (nullable, not throwing — snippets are optional).
- Platform actuals for `readSnippetResource`: desktopMain + androidMain use JVM classloader (return null on miss); iosMain + wasmJsMain return null (stubs, deferred to Phase 7).
- `shared/src/commonMain/resources/snippets/markdown/snippets.json` — bundled Table + Link snippets (Yole-authored, adapted from Microsoft vscode/markdown-basics MIT snippets).
- Tests: `VsCodeSnippetParserTest` (4 tests, commonTest) + `SnippetRegistryTest` (2 tests, desktopTest). **6/6 PASS** on `:shared:desktopTest`.
- Mutation verification (CONST-035): (1) parseBody→null: 2/4 FAILED; reverted. (2) forLanguage→emptyList(): 1/2 FAILED; reverted.
- KNOWN_DEFECTS.md: extended `#f2-phase-3-bonede-query-api-gap` with point 4 noting iOS+Wasm snippet bundling deferred to iter-60 Phase 7.
- Plan deviation: SnippetRegistry uses expect/actual (not JVM-only ::class.java.classLoader) because commonMain cannot reference JVM types. Mirrors iter-58 ScmQueryLoader pattern. Separate androidMain actual mirrors desktopMain (both JVM classloader).

**Next: Phase 3** — 3 CompletionProvider implementations: SnippetCompletionProvider (matches snippets.prefix against CompletionContext.prefix), TokenCompletionProvider (word-frequency index from document text), and KeywordCompletionProvider (static keyword list per langId). All in commonMain. Per plan Tasks 3.1–3.4. Pre-execution gate: Phase 2 complete (done). Then Phase 4 (engine + ranker) → Phase 5 (trigger) → Phase 6 (UI popup) → Phase 7 (50+ lang snippet bundles) → Phase 8 (placeholder navigation) → Phase 9 (challenges) → Phase 10 (docs) → Phase 11 (Firebase distribution v1.3.0).
