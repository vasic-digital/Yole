...
**Last updated:** 2026-05-20 (iter-84 EMERGENCY COMPLETE — v2.0.1: 3 Web Wasm bugs fixed (container ID + script tag + Shadow DOM splash poll), 5 Compose layout fixes, render gate PASS locally + at https://yole-app.web.app. Android v2.0.1 distributed to Firebase App Distribution (Release: 2o3olubl51ngo, Debug: 0931bg5vi6b2g). Desktop macOS DMG 525MB staged. All 15 iter-gates PASS. Committed SHA 6e60587f. Tagged v2.0.1. **iter-84 follow-up SHA 4f251802**: splash-fallback bumped 10s → 25s because the new render gate caught canvas mounting at 11.2s, after the 10s splash-fallback fired. Symptom for users: splash stayed visible forever even though the canvas had actually mounted behind it. Re-deployed; gate re-verified PASS with no console-error WARN. **Submodule resync SHAs 09bdb0e9 + f28c64e6**: pulled 4-7 upstream commits in each of 9 shared submodules (incl. HelixConstitution §11.4.40 + §11.4.41 mandates, CONST-061 cascade, anti-bluff round-17/19/28 sweeps); discarded 3 duplicate doc commits (content already on upstream under lowercase rename); Containers gitlab mirror reconciled via merge-first per the new §11.4.41 mandate. All shared-submodule upstreams (github + gitlab + HelixDevelopment) now in sync. **Hygiene + restore SHA 43f0b350 + parent bump SHA TBD**: added 3 gitignore patterns covering 6 long-standing untracked items (`*.bak-*`, `*-bak`, stray `/<module>/app/` scaffold dirs); restored Yole's iter-76 HelixQA feature-coverage scenarios + fixtures + coverage-matrix.md into Challenges (operator's parallel session had merge-deleted them), restored iter-77/78 iOS+macOS VM packages into Containers, gitignored Containers' `/crossbuild-matrix` Go binary. iter-76 scenario coverage gate re-runs PASS 25/0/1. Parent pointers bumped Challenges 1e08f4e5→4ae95e19, Containers 184c86a5→a4bde3a4, Security 5e547ce4→661ccdf4. All shared-submodule remotes (github + gitlab) in sync. **iter-85 EMERGENCY SHA ace84da1**: user reported "still opens just blank white page" after iter-84 PASS — META-BLUFF in iter-84 gate (PNG byte-count heuristic falsely PASSed a 154px-canvas-on-800px-viewport). Two interlocking fixes: (a) index.html CSS selector `#root` → `#yoleCanvas` (rename was incomplete in iter-84), canvas now fills viewport; (b) render-gate.js replaced byte heuristic with 3 hard assertions (canvas dimensions ≥ 80% viewport, 9-point viewport probe via elementFromPoint, decoded-pixel bottom-half color count via minimal inline PNG decoder). Gate now FAILs on iter-85 forensic case before fix, PASSes after. **iter-85 phase-2 SHA a7718e2d**: containerized web app testing via rootless podman (`make web-container-up`); full-UI accessibility-tree suite (`tools/node-render-gate/full-ui-suite.js`) asserts 19 positive UI elements + 2 negative leak patterns; surfaced + fixed real bug — Markdown preview pane was rendering literal CSS stylesheet (`.markdown { font-family: ... }`) as plain text because parseHtmlBlocks() didn't strip <style> blocks; preview now shows actual rendered content; gate wired as qa-iter-85-gates → qa-all (now 16 iter-gates). **iter-85 phase-3 SHA 341a8b3d**: closed `#iter-85-preview-styling-polish` — preview text was concatenated into one blob because outer `<div class='markdown'>` wrapper made parseHtmlBlocks treat all nested h1/p/ul/li as flat-text container. Fixed via outer-div peel + recursive div descent + explicit <li> extraction. Added 8 PREVIEW_REQUIRED standalone-node assertions + 2 more FORBIDDEN concatenation-leak substrings to the gate. Preview now renders proper h1/h2/bulleted-list/paragraphs. **iter-85 phase-4 SHA a0be3507**: codified anti-bluff covenant cascade audit as a permanent gate (`yole-challenges/scripts/anti_bluff_cascade_audit_challenge.sh`) — walks parent + 10 owned submodules × 4 governance files (44 total), asserts each contains a canonical covenant anchor (CONST-035/CONST-039/§11.4/verbatim quote). Current state: 44/44 PASS. Wired into qa-iter-85-gates. **iter-86 SHA 392b0c7c**: closed `#iter-82-completion-engine-k2-stub` — autocomplete CompletionEngineFlow was returning `emptyFlow()` due to two K2 FIR compiler crashes; replaced with manual `object : Flow<CompletionList>` impl that sidesteps both K2 crashes. 5 previously-FAILing CompletionEngineTest tests now PASS (full `:shared:desktopTest` suite green). Autocomplete now works on every Yole platform (Android/Desktop/iOS/Web). **iter-86 phase-2 SHA ddf329d5**: interactive-flow anti-bluff suite (`tools/node-render-gate/interactive-flow-suite.js`) wired as qa-iter-86-gates → qa-all (now 17 iter-gates). Clicks each toolbar button + asserts expected dialog/panel/state delta — catches dead buttons that pass inventory but do nothing on click. 8/8 flows PASS. **iter-87: v2.0.2 multi-platform release shipped**: bumped Android versionCode 201→202 + versionName 2.0.1→2.0.2, Desktop packageVersion → 2.0.2, iOS MARKETING_VERSION/CURRENT_PROJECT_VERSION → 2.0.2/202. Built + distributed: Android Release `Yole-Android-2.0.2-Release-0.0.0.2.2.apk` (44 MB) → Firebase App Distribution release `008qa5s23ul7o`, Android Debug `Yole-Android-2.0.2-Debug-0.0.0.2.2.apk` (56 MB) → Firebase App Distribution release `3t5j401rt22m8` (Yole DEV app `1:578988389676:android:5a3d47a9fb23b6465d2889`), Desktop macOS arm64 `Yole-Desktop-macos-arm64-2.0.2-Release-0.0.0.2.2.dmg` (526 MB) staged in `releases/`, Web Wasm rebuilt + deployed to https://yole-app.web.app + verified by all 3 web gates (render-gate + full-ui-suite + interactive-flow-suite). Forensic + permanent fix: `scripts/distribute.sh` now reads per-variant Firebase app IDs (`FIREBASE_ANDROID_APP_ID_RELEASE` / `FIREBASE_ANDROID_APP_ID_DEBUG`) — was a single-ID config that failed v2.0.2 debug upload with "APK package name 'digital.vasic.yole.android.dev' does not match Firebase app's 'digital.vasic.yole.android'". Tag `yole-2.0.2` pushed (avoiding 2019-Markor-upstream `v2.0.2` collision). **iter-88: Save button works again, closed `#iter-86-save-localstorage-tighten-assert`**: post-v2.0.2 anti-bluff probe of Save flow caught a silent crash — every Save click hit `IrLinkageError: Can not get instance of singleton 'System'` because kotlinx-datetime 0.6.1 wasm klib doesn't ship `Clock.System` (the iter-82 forceResolution pin to 0.6.1 was correct for JVM/Desktop but the wasm klib is missing Clock.System). Replaced all 6 `Clock.System.*` callsites in wasm-side code with `webApp/src/wasmJsMain/.../WebTime.kt` shim (`@JsFun` → `Date.now()` / `Date.toISOString()`). Interactive-flow Save check tightened from "page survives" to 4 explicit assertions (localStorage grew + `yole_web_state_content` + `yole_web_state_timestamp` + `yole_doc_*` blob). Verified: localStorage 0→8 keys after Save, 0 console errors. Shipped v2.0.3 — Android Release + Debug redistributed to Firebase, Desktop DMG staged, Web redeployed. Tag `yole-2.0.3`. **iter-89: SW stale-cache fix shipped as v2.0.4**: operator reported "spins endlessly" — root cause was Service Worker shipping `CACHE_NAME = 'yole-cache-v1'` static through 4 deploys. Returning users were served the v2.0.0 broken bundle from cache forever; web gates PASSed because fresh Puppeteer sessions had no prior cache. Fixed by version-aware CACHE_NAME, switching to network-first (fresh content always wins), adding skipWaiting+clients.claim for instant SW takeover. New permanent gate `web_sw_cache_version_challenge` (qa-iter-89-gates → qa-all, chain now 18 iter-gates) verifies SW source AND deployed SW both declare CACHE_VERSION matching versionName. Released as v2.0.4 — Android Release `56tm3fubeeiv0`, Debug `1ol15ck4iba3o`, Desktop DMG staged, Web redeployed. After this release, returning users see the fix on next page refresh (no hard refresh needed). **iter-90: Mobile responsiveness + launcher-icon anti-bluff (v2.0.5)**: operator-reported defect — desktop layout cramped into 320 px on mobile, toolbar buttons cut off, prior gates ran only at 1280×800 viewport so the mobile broken state was invisible. Fixed via `BoxWithConstraints` in EnhancedWebApp.kt with 3 breakpoints (compact <768 / medium 768-1023 / wide >=1024) that auto-collapse sidebar+preview on narrow widths; toolbar split into horizontally-scrollable left zone + pinned right zone (Toggle theme + Settings always visible). Two new permanent gates: `responsive-suite.js` probes 6 viewports (320 → 1280) asserting key UI reachable + breakpoint contract honored, `web_logo_presence_challenge.sh` verifies favicon + 6 PWA PNGs + SVG + Logo.png all return image content-type (defeats Firebase's `**` → /index.html rewrite that masks missing assets as HTTP 200) + page <title> contains 'Yole' + in-app YOLE logo a11y-tree present. New `docs/ENDPOINTS.md` documents all distribution URLs. Released as v2.0.5 — Android Release `7cff6nns6d5ig`, Debug `64r5nt0scocn8`, Desktop DMG staged, Web redeployed. qa-all chain is now 19 iter-gates. **iter-91: Android launcher icon visibility fix (v2.0.6)**: post-v2.0.5 operator-equivalent verification — installed Release APK on emulator-5554 and captured screenshot of system "All files access" page. Found launcher icon rendering as BLANK white circle (Y invisible). Root cause: ic_launcher_foreground.xml fillColor was `#FFFFFFFF` (white) on `#FFFFFFFF` (white) background — iter-71 assumed the foreground layer was tinted by the system but only the MONOCHROME layer is tinted. Bug shipped invisibly across v2.0.0 → v2.0.5 (five release cycles). installable_app_icon_challenge had three layers (source/APK/vector structure) — none checked color contrast — so it PASSed throughout. Fixed: fillColor `#FFFFFFFF` → `#FFD32F2F` (Yole red). Gate hardened with new LAYER D: parses foreground vector's `<path>` fillColor via Python XML (avoids matching commented-out hex literals) + adaptive icon's @color/ background reference + resolves to colors.xml, asserts FG ≠ BG. Verified on-device: emulator screenshot shows visible RED Y in white circle. Anti-bluff: gate FAILs without fix, PASSes after. Released as v2.0.6 — Android Release `1qep5papjgmo8`, Debug `11ac9q6ck1olo`, Desktop DMG staged, Web redeployed. Full 19-gate qa-all sweep verified PASS. One open tracker remains: `#iter-85-helixqa-autonomous-needs-llm-keys` (operator-side LLM API keys). **codegraph epic — Phase 0 COMPLETE (2026-05-20)**: the 4 shared HelixDevelopment submodules (DocProcessor / LLMOrchestrator / VisionEngine / LLMsVerifier) had stale uncommitted doc rewrites sitting 22–47 commits behind fast-moving upstreams; each was rebased onto its fresh origin tip via git plumbing (read-tree / write-tree / commit-tree — avoids the macOS case-collision that blocks `git rebase`), the 3 `ARCHITECTURE.md` docs were corrected against the real `pkg/` trees (wrong `LLMAgent`/`OpenCode` signatures, dropped `MultiProviderPool`/`pkg/remote` packages, hallucinated parser/OpenCV files — all fixed), a generic `internal/archdoc` doc-accuracy test was added to each Go submodule (fails the build on any future doc drift), the macOS-fatal `docs/ARCHITECTURE.md` vs `docs/architecture.md` case-collision was deduplicated to the uppercase canonical, a jsdom anti-bluff smoke test was added to LLMsVerifier, and all 4 were pushed to their upstreams. Epic spec: `docs/superpowers/specs/2026-05-20-codegraph-integration-and-anti-bluff-epic-design.md`; Phase 0 plan: `docs/superpowers/plans/2026-05-20-phase-0-submodule-doc-merge.md`. **codegraph epic Phases 1-4 COMPLETE (2026-05-20)**: codegraph v0.6.8 installed and MCP-wired into all 5 CLI agents — Claude Code + Qwen Code project-scoped (`.mcp.json` + `.qwen/settings.json`, committed), OpenCode + Crush + Kimi user-global — all verified connected. The Yole codebase is indexed (911 files / 21,964 symbols) into `.codegraph/codegraph.db` (git-ignored; `.codegraph/config.json` committed, scoped to Yole-own code, submodules + `tools/opensource` excluded). Two host issues found + worked around: Node 23+ OOMs the tree-sitter WASM parser (`Fatal process out of memory: Zone`) so `scripts/codegraph-index.sh` runs indexing on Node 18-22; and the native vs WASM SQLite backends conflict on WAL mode, so native `better-sqlite3` is forced off for one consistent WASM-SQLite backend. `docs/CODEGRAPH.md` documents the integration + reinstall steps. Anti-bluff Challenge `yole-challenges/scripts/codegraph_integration_challenge.sh` (10/10 PASS — real index-DB query + real MCP JSON-RPC `tools/list` handshake returning 8 tools) is wired into `make qa-all` via `make codegraph-verify`. NEXT = Phase 5 (full anti-bluff re-audit of ~9,400 tests + ~25 challenges) and Phase 6 (anti-bluff covenant + QWEN.md governance propagation across HelixConstitution + owned submodules). **codegraph epic — Phase 5 IN PROGRESS (2026-05-20)**: full anti-bluff re-audit. Plan: `docs/superpowers/plans/2026-05-20-phase-5-anti-bluff-reaudit.md`. **Phase 5A (Pitest infra) — DONE except baseline:** Pitest 1.15.0 wired into `:shared` (commits 2eb5a1c2 / 4c455fbf / a7323e45 — `pitestTool` configuration + `pitestToolClasspath`/`pitestClasspath` Gradle tasks; no JVM-only Gradle plugin since a KMP module has no Java source sets); `scripts/anti-bluff/run-pitest.sh` created (commit 1599ba94, full/changed/`<pkg>` modes); THREE real defects found + fixed before a clean baseline run was possible: (1) commit c3d48172 — Pitest `--classPath` needs COMMA separation not colon (colon-form made Pitest scan 0 mutable classes); (2) c3d48172 — `--skipFailingTests true` added because ~21 timing-sensitive tests pass under Gradle but fail in Pitest's forked minion JVM; (3) commits 306117ba + 1bb49842 — `--mutableCodePaths "${CLASSES_MAIN}"` (absolute path) added because production and test classes share the `digital.vasic.yole.*` namespace, so the first run mutated TEST classes (a test mutating its own body trivially self-kills → meaningless inflated data). Each fix smoke-verified; csv smoke = 62 mutations over production classes only, 55 KILLED/3 SURVIVED/4 NO_COVERAGE = 88%. The clean per-package full Pitest baseline run is IN PROGRESS (background agent) — populates `bluff-baseline.txt` Section 2 with honest per-package kill-rate floors (~4-7 h wall-clock; each package run does a full-suite coverage pass). **Phase 5A baseline — COMPLETE (commit fd84f817):** after the 3 machinery defects, a clean per-package Pitest pass produced `bluff-baseline.txt` Section 2 with real floors for all 53 production sub-packages — 0 test-class contamination, overall 6281/14647 = 42% mutation kill rate (a hard quantification of the operator's "most features don't work" concern). The 42% floor independently corroborates the 5C audit: weakest packages (network.protocols.ftp 19%, git 24%, ui 19%, database 0%) are exactly the audit's bluff-heavy areas. Baseline runner committed as reusable infra: `scripts/anti-bluff/run-pitest-baseline.sh` (stable XML store under `shared/build/pitest-baseline-xml/`; detected = KILLED+TIMED_OUT+MEMORY_ERROR+RUN_ERROR). **Phase 5B (real mutation_ratchet gate) — COMPLETE + PROVEN (commit c5dbdafe):** `mutation_ratchet_challenge.sh` rewritten — runs real Pitest, ratchets per-package detected-rates vs Section 2, FAILs a package below floor; `run-pitest.sh` changed-mode now also maps changed TEST files to their production package (weakened-test bluff vector); Makefile `anti-bluff-mutation`=changed-mode (qa-all), `mutation-full`=full check, `mutation-baseline`=floor refresh; `SurvivingMutantBluff.kt` fixture added. End-to-end PROVEN: changed-mode no-op passes clean; with the csv floor tampered to 99% the gate ran real Pitest, measured csv 88%, and FAILed exit 1 (auto-restored). Closes CRITICAL P5-FIX-144 (the gate was itself a bluff stub). NOTE: a zombie background subagent (addf6392) caused a multi-hour incident — it kept respawning Pitest runs after TaskStop reported it "completed", contaminating report dirs; resolved by letting it finish naturally then topping up. Lesson: drive long sequential job-loops with a committed shell script as ONE tracked background command, never a subagent. **Phase 5C (audit) — COMPLETE:** 15 report-only subagent batches (B01-B15) audited all 466 units (427 test files + 39 challenge scripts); per-batch findings in `docs/superpowers/audits/phase-5/B*.md`; **55 BLUFF + 103 SUSPECT** consolidated into `docs/superpowers/audits/phase-5/CONSOLIDATED.md` as 158 `P5-FIX-NNN` items across **12 systemic patterns** (PAT-01 `assertTrue(result.isSuccess||result.isFailure)` tautology ~40×, PAT-03 `assertNotNull`-only ~20×, PAT-04 self-verifying test doubles ~14×, PAT-02 performance-only ~12×, etc.) plus **10 CRITICAL** items possibly hiding genuinely broken features (XSS escaping unverified across all 17 parsers, path-traversal protection unverified, all 5 LSP requester features exercised only by self-fakes, auth-token races invisible, Firebase init unverified, the mutation_ratchet stub). NOTE: B12/B14/B15 were re-audited with fresh agents after a first subagent went off-script and fabricated verdicts for files it never opened — the corrected re-audits + the `README.md` index are the authoritative versions. Audit commits: 85b8082d, 098ec482, 57e2b028. **Phase 5D operator checkpoint — COMPLETE:** operator decided Phase 5E fixes **all 158** P5-FIX items — nothing deferred, full anti-bluff covenant compliance (~4-5 engineer-weeks, spans many sessions). **Phase 5E (fix sweep) — IN PROGRESS:** executed as pattern campaigns in CONSOLIDATED.md Section 5 priority order — CRITICAL-first (each CRITICAL fix MUST first verify the real feature works and fix the PRODUCT if it is broken, per the covenant), then mechanical pattern sweeps, then independent investigations; fix subagents run sequentially because they edit code (unlike the parallel report-only audit subagents). **NEXT — Phase 5E fix sweep (all 158 P5-FIX items, ~4-5 engineer-weeks, multi-session).** Execute as pattern campaigns in `CONSOLIDATED.md` Section 5 priority order. Campaign 1 = XSS security fix (P5-FIX-001 `FormatParserSecurityTests` + P5-FIX-089 `SecurityValidationTests` — both have XSS-check helpers that assert on a hardcoded string instead of real parser `toHtml()` output; the fix MUST first verify whether the 17 parsers actually escape HTML and fix the PRODUCT if any leak raw `<script>`). Then campaigns 2-N per CONSOLIDATED.md. Each fix is TDD: strengthen the test so it FAILs against a stubbed/non-escaping unit, fix root cause (product if the feature is genuinely broken, per the operator covenant), confirm PASS, then `MUTATION_MODE='<pkg>' mutation_ratchet_challenge.sh` to confirm and raise that package's Section-2 floor. Fix subagents run SEQUENTIALLY (they edit code) — never parallel unless partitioned to disjoint files. After 5E: Phase 5F = full gate sweep + close-out. Phase 6 (governance propagation) follows Phase 5. **Phase 5E Campaign 1 (XSS) — COMPLETE (2026-05-21):** P5-FIX-001 + P5-FIX-089 fixed. The strengthened tests caught a REAL product XSS defect in 3 parsers (OrgMode / RMarkdown / Jupyter) — they emitted raw `<script>` from attacker document content into `toHtml()`. Product fixed (escape-before-format in `OrgModeParser`/`RMarkdownParser`, escape fallback + metadata + cell source in `JupyterParser`); tests strengthened to assert on real `doc.toHtml()`. 121/121 security tests PASS; tamper-revert proved non-bluff. See Section 76. **Phase 5E Campaign 1 follow-up — COMPLETE (2026-05-21):** a security review of `19d73dad` found the OrgMode fix incomplete — live stored-XSS remained at 3 raw interpolations in `generateOrgHtml()` (block header + property drawer + standalone property) and `sanitizeUri()` was bypassable via interior-whitespace (`java<TAB>script:`). Fixed C1 (escape all 3 sites, shared `propertyRowHtml()` helper), C2 (`sanitizeUri()` rewritten as a whitespace-stripping allowlist), I1 (corrected the false Jupyter "rendered raw" narrative + registered `#phase5-jupyter-html-sanitizer`), I2 (4 new OrgMode-branch tests + `\bon[a-z]+\s*=` event-handler regex). The 4 new tests FAIL on `19d73dad`, PASS after the fix; full `:shared:desktopTest` green. See Section 77.)

## Section 77 — Phase 5E Campaign 1 follow-up: OrgMode XSS gap closed (security review C1/C2/I1/I2)

**Status:** COMPLETE.

**Branch:** `master` (one follow-up commit on top of `19d73dad`).

### Summary

A security review of Campaign 1 commit `19d73dad` found the OrgMode fix was
**incomplete — live stored-XSS remained**. The original fix only escaped the
heading/paragraph path (`formatInlineOrg`); three other raw interpolations in
`generateOrgHtml()` were untouched, and `sanitizeUri()` used a bypassable
denylist.

### Issues fixed

- **C1 (CRITICAL)** — `generateOrgHtml()` emitted raw attacker content at three
  sites: the block header (`#+BEGIN_SRC <script>...`), property-drawer lines,
  and standalone property references. All three now `escapeHtml()` their inputs
  (drawer + standalone share a new `propertyRowHtml()` helper).
- **C2 (CRITICAL)** — `sanitizeUri()` rewritten from a denylist to an
  **allowlist**: strips all whitespace + ASCII control chars first (defeats the
  `java<TAB>script:` interior-whitespace bypass), then accepts only
  `http`/`https`/`mailto` schemes, in-page anchors, and relative/scheme-less
  paths; everything else returns `#`.
- **I1 (Important)** — corrected the false "rendered raw" narrative for Jupyter
  `text/html` output (it is entity-escaped — safe but degrades rich output);
  added a `TODO(#phase5-jupyter-html-sanitizer)` marker and registered the
  tracker in `docs/KNOWN_DEFECTS.md`.
- **I2 (Important)** — added 4 OrgMode-specific tests (block-header,
  property-drawer, standalone-property, interior-whitespace `javascript:` URI)
  that drive the previously-uncovered branches; broadened the event-handler
  detector from a 5-name list to a `\bon[a-z]+\s*=` regex.

### Anti-bluff proof

The 4 new OrgMode tests FAIL against unfixed `19d73dad` code (block-header,
property-drawer, standalone-property, tab-URI) and PASS after the C1/C2 fixes.
Full `:shared:desktopTest` green; anti-bluff scanner clean.

### Files changed in the follow-up

| File | Change |
|------|--------|
| `shared/.../format/orgmode/OrgModeParser.kt` | C1 escape 3 raw sites + `propertyRowHtml()` helper; C2 allowlist `sanitizeUri()` |
| `shared/.../format/jupyter/JupyterParser.kt` | I1 `TODO(#phase5-jupyter-html-sanitizer)` marker |
| `shared/.../format/coverage/FormatParserSecurityTests.kt` | I2 four new OrgMode tests + regex event-handler detector |
| `docs/KNOWN_DEFECTS.md` | I1 `#phase5-jupyter-html-sanitizer` tracker registered |

## Section 76 — Phase 5E Campaign 1: XSS security fix (P5-FIX-001 + P5-FIX-089)
...
The XSS-prevention tests were bluff: `FormatParserSecurityTests.verifyXssPrevention`/
`verifyHtmlInjection` asserted on `payload.escapeHtml()` (a utility string) instead of
the parser's real `doc.toHtml()` output; `SecurityValidationTests.assertNoRawScript`
ignored its `html` argument and tested a hardcoded constant. Strengthening the helpers
to inspect real `toHtml()` output uncovered a **genuine product XSS defect** in 3 of 17
parsers: OrgMode, RMarkdown, Jupyter emitted raw `<script>` / event handlers from
attacker-controlled document content (stored XSS when a user opens the document).

### Product fix

- `OrgModeParser` — `formatInlineOrg()` now HTML-escapes text before inline markup;
  added `sanitizeUri()` to neutralize `javascript:`/`data:`/`vbscript:` link hrefs.
- `RMarkdownParser` — `generateContentHtml()` escapes content before markdown regex
  processing (removed now-redundant double-escape of fenced code); `generateCodeChunkHtml()`
  escapes chunk code+language; `toHtml()` escapes front-matter title.
- `JupyterParser` — escapes the JSON-parse-failure fallback body, notebook
  title/kernel/language metadata, and markdown/raw cell source. (Residual: executed-cell
  `text/html` output is intentionally rendered raw as a notebook feature — out of scope.)

### Test strengthening

Both bluff helpers rewritten to inspect real `doc.toHtml()` — fail on an executable
`<script>` tag, a live inline event handler inside an unescaped tag, or a live
`javascript:` URI. Tamper-revert proof: reverting the OrgMode escape to a no-op makes
exactly the 3 OrgMode XSS tests FAIL; restored → 121/121 PASS.

### Files changed in Campaign 1

| File | Change |
|------|--------|
| `shared/.../format/orgmode/OrgModeParser.kt` | escape-before-format + URI sanitizer (product fix) |
| `shared/.../format/rmarkdown/RMarkdownParser.kt` | escape content/chunk/title (product fix) |
| `shared/.../format/jupyter/JupyterParser.kt` | escape fallback/metadata/cell source (product fix) |
| `shared/.../format/coverage/FormatParserSecurityTests.kt` | strengthened XSS/injection helpers |
| `shared/.../security/SecurityValidationTests.kt` | strengthened `assertNoRawScript` |
