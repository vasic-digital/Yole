...
**Last updated:** 2026-05-15 (iter-58 F2 Phase 1 COMPLETE — LanguageFormat + LanguageRegistry + LocalLanguage foundation landed. New files in `shared/src/commonMain/kotlin/digital/vasic/yole/language/`: `LanguageFormat.kt` (data class carrying id, displayName, extensions, mimeTypes, commentSyntax, indentRules, bracketPairs, indentUnit), `LanguageMetadata.kt` (static manifest — markdown + kotlin as Phase 1 anchors; Phase 6 fills other 48+), `LanguageRegistry.kt` (get/detectByFilename/all), `LocalLanguage.kt` (Compose CompositionLocal<LanguageFormat?> mirroring LocalTheme pattern). Stub affordance data classes: `affordance/CommentSyntax.kt`, `affordance/IndentRules.kt`, `affordance/BracketPairs.kt` (Phase 2 adds method logic). Test: `shared/src/commonTest/.../language/LanguageRegistryTest.kt` — 4 cases (markdownIsRegistered, detectByFilename_handlesKotlin, detectByFilename_returnsNullOnUnknownExtension, all_listsAtLeastMarkdown). Anti-bluff (CONST-035): mutation — stub detectByFilename to return null → detectByFilename_handlesKotlin FAILS; reverted, 4/4 PASS. Cross-platform impact (CONST-037): pure commonMain, identical on all 4 platforms. Next: F2 Phase 2 — CommentSyntax.toggleLine + IndentRules.computeIndent + BracketPairs.closerFor + their tests.)
