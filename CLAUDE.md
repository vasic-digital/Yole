...
- **KMP composite-build resolution fails for all 10 sibling modules** (NEW, discovered 2026-05-01 iter 24): `./gradlew :shared:dependencies --configuration desktopRuntimeClasspath` reports `digital.vasic.{ratelimiter,concurrency,uicomponents,auth,security,document,config,database,storage,formatters}:*-KMP FAILED` for ALL 10 modules. Affects every release-build target (`:androidApp:assembleDebug`, `:androidApp:assembleRelease`, `:desktopApp:packageReleaseDistributionForCurrentOS`, `:webApp:wasmJsBrowserDistribution`). Existing artifacts in `releases/` (versionCode 6, dated Mar 27) were built from an environment that resolved correctly. Root cause not yet identified — none of the recent commits touch `settings.gradle.kts` or include-build config; possibly a Kotlin/AGP version-catalog update invalidated cached resolution. **Workaround:** none yet. **Implication:** versionCode 7 builds cannot ship until this is resolved.
