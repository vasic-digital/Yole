...
# Either form, on the same line as the offending statement OR on the
# immediately preceding line, suppresses the hit.
...
scan_go() {
  ...
  awk -v rel="${relpath}" '
    {
      ...
      cur_exempt = (index($0, "SKIP-OK:") > 0 || index($0, "ANTI-BLUFF-EXEMPT:") > 0)
  ...
  awk -v rel="${relpath}" '
    /^[[:space:]]*assert\.True\(t,[[:space:]]*true\)/ {
      ...
      if (index($0, "SKIP-OK:") == 0 && index($0, "ANTI-BLUFF-EXEMPT:") == 0 &&
          (prev !~ /SKIP-OK:|ANTI-BLUFF-EXEMPT:/)) {
  ...
  #
  # Precision fix (iter 13): pattern matching is done on the line with string
  # and rune literals stripped, so `t.Logf("describes assert.something")`
  # doesn't trick `assert\.` matcher. Brace counting is also done after
  # stripping so `}` inside a string literal doesn't decrement brace depth.
  awk -v rel="${relpath}" '
    function strip_literals(line,  out, c, i, in_str, in_raw, esc) {
      # Remove "..." (with \-escapes), `...`, and rune literals like '"'"'\n'"'"'.
      # Returns the line with literal contents replaced by empty quotes,
      # preserving structure (e.g., `"abc"` becomes `""`). Comments are also
      ...
      for (i = 1; i <= length(line); i++) {
        ...
        if (c == "/" && substr(line, i, 2) == "//") { break }  # line comment
        out = out c
      }
      return out
    }
