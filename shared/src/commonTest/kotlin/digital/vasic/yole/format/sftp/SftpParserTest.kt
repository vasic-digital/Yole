...
/**
 * Unit tests for SFTP format parser.
 *
 * Tests cover:
...
class SftpParserTest {
    ...
    @Test
    fun `should detect SFTP format by extension`() {
        val format = FormatRegistry.getByExtension(".sftp")

        assertNotNull(format)
        assertThat(format.id).isEqualTo(FormatRegistry.ID_SFTP)
        assertThat(format.name).isEqualTo("SFTP")
    }

    @Test
    fun `should detect SFTP format by filename`() {
        val format = FormatRegistry.detectByFilename("test.sftp")
    ...
    @Test
    fun `should support all SFTP extensions`() {
        val extensions = listOf(".sftp")
    ...
    @Test
    fun `should parse basic SFTP content`() {
        val content = """
            Sample SFTP content here
        """.trimIndent()
    ...
    fun `should handle single line input`() {
        val content = "Single line of SFTP"
    ...
    fun `should not false-positive on plain text`() {
        ...
        // Should detect as plaintext, not SFTP
        if (format != null) {
            assertThat(format.id).isNotEqualTo(FormatRegistry.ID_SFTP)
        }
    }
    ...
    fun `should handle very long input`() {
        val longContent = "Single line of SFTP\n".repeat(10000)
    ...
    fun `should integrate with FormatRegistry`() {
        val format = FormatRegistry.getById(FormatRegistry.ID_SFTP)

        assertNotNull(format)
        assertThat(format.name).isEqualTo("SFTP")
        assertThat(format.defaultExtension).isEqualTo(".sftp")
    }

    @Test
    fun `should be registered in FormatRegistry`() {
        val allFormats = FormatRegistry.formats
        val sftpFormat = allFormats.find { it.id == FormatRegistry.ID_SFTP }

        assertNotNull(sftpFormat)
        assertThat(sftpFormat.name).isEqualTo("SFTP")
    }
}
