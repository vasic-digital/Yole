 ...
 *
 * Integration tests for CSV format
 *
 *########################################################*/
...
class CsvIntegrationTest {
    ...
    fun `content detection should match extension detection`() {
        ...
        val byExtension = FormatRegistry.getByExtension(".csv")
    ...
    fun `should not conflict with other formats`() {
        ...
        // Ensure CSV format is unique
        val csvFormats = formats.filter { it.id == FormatRegistry.ID_CSV }
        assertThat(csvFormats).hasSize(1)
    }
    ...
    fun `should work with file-like operations`() {
        ...
        val format = FormatRegistry.detectByFilename("document.csv")
        assertNotNull(format)
