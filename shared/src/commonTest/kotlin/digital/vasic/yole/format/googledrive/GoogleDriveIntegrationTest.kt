 ...
 *
 * Integration tests for GoogleDrive format
 *
 *########################################################*/
...
class GoogleDriveIntegrationTest {
    ...
    fun `content detection should match extension detection`() {
        ...
        val byExtension = FormatRegistry.getByExtension(".gdoc")
    ...
    fun `should not conflict with other formats`() {
        ...
        // Ensure GoogleDrive format is unique
        val googledriveFormats = formats.filter { it.id == FormatRegistry.ID_GOOGLEDRIVE }
        assertThat(googledriveFormats).hasSize(1)
    }
    ...
    fun `should work with file-like operations`() {
        ...
        val format = FormatRegistry.detectByFilename("document.gdoc")
        assertNotNull(format)
    ...
    fun `should handle large files efficiently`() {
        val largeContent = buildString {
            repeat(1000) {
                appendLine("Single line of GoogleDrive")
            }
        }
