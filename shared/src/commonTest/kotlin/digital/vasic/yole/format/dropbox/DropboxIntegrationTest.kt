 ...
 *
 * Integration tests for Dropbox format
 *
 *########################################################*/
...
class DropboxIntegrationTest {
    ...
    fun `content detection should match extension detection`() {
        ...
        val byExtension = FormatRegistry.getByExtension(".dbx")
    ...
    fun `should not conflict with other formats`() {
        ...
        // Ensure Dropbox format is unique
        val dropboxFormats = formats.filter { it.id == FormatRegistry.ID_DROPBOX }
        assertThat(dropboxFormats).hasSize(1)
    }
    ...
    fun `should work with file-like operations`() {
        ...
        val format = FormatRegistry.detectByFilename("document.dbx")
        assertNotNull(format)
    ...
    fun `should handle large files efficiently`() {
        val largeContent = buildString {
            repeat(1000) {
                appendLine("Single line of Dropbox")
            }
        }
