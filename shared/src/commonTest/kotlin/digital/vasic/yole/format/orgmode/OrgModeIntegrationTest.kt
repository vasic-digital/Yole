 ...
 *
 * Integration tests for Org Mode format
 *
 *########################################################*/
...
class OrgModeIntegrationTest {
    ...
    fun `content detection should match extension detection`() {
        ...
        val byExtension = FormatRegistry.getByExtension(".org")
    ...
    fun `should not conflict with other formats`() {
        ...
        // Ensure Org Mode format is unique
        val org modeFormats = formats.filter { it.id == FormatRegistry.ID_ORG_MODE }
        assertThat(org modeFormats).hasSize(1)
    }
    ...
    fun `should work with file-like operations`() {
        ...
        val format = FormatRegistry.detectByFilename("document.org")
        assertNotNull(format)
