 ...
 *
 * Integration tests for SFTP format
 *
 *########################################################*/
...
class SftpIntegrationTest {
    ...
    fun `content detection should match extension detection`() {
        ...
        val byExtension = FormatRegistry.getByExtension(".sftp")
    ...
    fun `should not conflict with other formats`() {
        ...
        // Ensure SFTP format is unique
        val sftpFormats = formats.filter { it.id == FormatRegistry.ID_SFTP }
        assertThat(sftpFormats).hasSize(1)
    }
    ...
    fun `should work with file-like operations`() {
        ...
        val format = FormatRegistry.detectByFilename("document.sftp")
        assertNotNull(format)
    ...
    fun `should handle large files efficiently`() {
        val largeContent = buildString {
            repeat(1000) {
                appendLine("Single line of SFTP")
            }
        }
