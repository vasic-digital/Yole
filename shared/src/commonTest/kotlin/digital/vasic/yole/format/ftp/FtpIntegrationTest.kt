 ...
 *
 * Integration tests for FTP format
 *
 *########################################################*/
...
class FtpIntegrationTest {
    ...
    fun `content detection should match extension detection`() {
        ...
        val byExtension = FormatRegistry.getByExtension(".ftp")
    ...
    fun `should not conflict with other formats`() {
        ...
        // Ensure FTP format is unique
        val ftpFormats = formats.filter { it.id == FormatRegistry.ID_FTP }
        assertThat(ftpFormats).hasSize(1)
    }
    ...
    fun `should work with file-like operations`() {
        ...
        val format = FormatRegistry.detectByFilename("document.ftp")
        assertNotNull(format)
