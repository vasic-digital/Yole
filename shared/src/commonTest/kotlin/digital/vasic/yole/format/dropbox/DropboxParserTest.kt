...
/**
 * Unit tests for Dropbox format parser.
 *
 * Tests cover:
...
class DropboxParserTest {
    ...
    @Test
    fun `should detect Dropbox format by extension`() {
        val format = FormatRegistry.getByExtension(".dbx")

        assertNotNull(format)
        assertThat(format.id).isEqualTo(FormatRegistry.ID_DROPBOX)
        assertThat(format.name).isEqualTo("Dropbox")
    }

    @Test
    fun `should detect Dropbox format by filename`() {
        val format = FormatRegistry.detectByFilename("test.dbx")
    ...
    @Test
    fun `should support all Dropbox extensions`() {
        val extensions = listOf(".dbx")
    ...
    @Test
    fun `should parse basic Dropbox content`() {
        val content = """
            Sample Dropbox content here
        """.trimIndent()
    ...
    fun `should handle single line input`() {
        val content = "Single line of Dropbox"
    ...
    fun `should not false-positive on plain text`() {
        ...
        // Should detect as plaintext, not Dropbox
        if (format != null) {
            assertThat(format.id).isNotEqualTo(FormatRegistry.ID_DROPBOX)
        }
    }
    ...
    fun `should handle very long input`() {
        val longContent = "Single line of Dropbox\n".repeat(10000)
    ...
    fun `should integrate with FormatRegistry`() {
        val format = FormatRegistry.getById(FormatRegistry.ID_DROPBOX)

        assertNotNull(format)
        assertThat(format.name).isEqualTo("Dropbox")
        assertThat(format.defaultExtension).isEqualTo(".dbx")
    }

    @Test
    fun `should be registered in FormatRegistry`() {
        val allFormats = FormatRegistry.formats
        val dropboxFormat = allFormats.find { it.id == FormatRegistry.ID_DROPBOX }

        assertNotNull(dropboxFormat)
        assertThat(dropboxFormat.name).isEqualTo("Dropbox")
    }
}
