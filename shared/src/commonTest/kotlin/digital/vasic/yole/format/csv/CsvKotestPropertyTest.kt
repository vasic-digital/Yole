 ...
 *
 * Property-based tests using Kotest for CSV format
 *
 *########################################################*/
...
/**
 * Property-based tests for CSV format using Kotest.
 *
 ...
 */
class CsvPropertyTest : StringSpec({

    val parser = CsvParser()

    // ==================== Basic Property Tests ====================

    "parsing any valid CSV content should not throw exceptions" {
        checkAll(validCsvContent()) { content ->
    ...
    "parsing then serializing should preserve content structure" {
        checkAll(validCsvContent()) { original ->
    ...
    "parser should handle different line ending styles" {
        checkAll(validCsvContent(), Arb.lineEnding()) { content, lineEnding ->
    ...
    "should maintain format-specific property 1" {
        checkAll(formatSpecificCsvContent()) { input ->
    ...
    "parser should gracefully handle malformed input" {
        checkAll(malformedCsvContent()) { malformed ->
    ...
    "parser should handle truncated content" {
        checkAll(validCsvContent(), Arb.int(0..100)) { content, cutoff ->
    ...
    "parsing large content should complete in reasonable time" {
        checkAll(10, largeValidCsvContent()) { largeContent ->
...
/**
 * Generates valid CSV content samples.
 */
fun validCsvContent(): Arb<String> = arbitrary { rs ->
    val samples = listOf(
...
/**
 * Generates large valid CSV content for performance testing.
 */
fun largeValidCsvContent(): Arb<String> = arbitrary { rs ->
    buildString {
        repeat(rs.random.nextInt(500, 2000)) {
            appendLine("Single line of CSV")
        }
    }
}
...
/**
 * Generates malformed CSV content for error handling tests.
 */
fun malformedCsvContent(): Arb<String> = arbitrary { rs ->
 ...
 */
fun formatSpecificCsvContent(): Arb<String> = arbitrary { rs ->
