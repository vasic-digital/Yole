...
/**
 * Unit tests for FTP format parser.
 *
 * Tests cover:
...
class FtpParserTest {
    ...
    @Test
    fun `should detect FTP format by extension`() {
        val format = FormatRegistry.getByExtension(".ftp")

        assertNotNull(format)
        assertThat(format.id).isEqualTo(FormatRegistry.ID_FTP)
        assertThat(format.name).isEqualTo("FTP")
    }

    @Test
    fun `should detect FTP format by filename`() {
        val format = FormatRegistry.detectByFilename("test.ftp")
    ...
    @Test
    fun `should support all FTP extensions`() {
        val extensions = listOf(".ftp")
    ...
    @Test
    fun `should parse basic FTP content`() {
        val content = """
            Sample FTP content here
        """.trimIndent()
    ...
    fun `should handle single line input`() {
        val content = "Single line of FTP"
    ...
    fun `should not false-positive on plain text`() {
        ...
        // Should detect as plaintext, not FTP
        if (format != null) {
            assertThat(format.id).isNotEqualTo(FormatRegistry.ID_FTP)
        }
    }
    ...
    fun `should handle very long input`() {
        val longContent = "Single line of FTP\n".repeat(10000)
    ...
    fun `should integrate with FormatRegistry`() {
        val format = FormatRegistry.getById(FormatRegistry.ID_FTP)

        assertNotNull(format)
        assertThat(format.name).isEqualTo("FTP")
        assertThat(format.defaultExtension).isEqualTo(".ftp")
    }

    @Test
    fun `should be registered in FormatRegistry`() {
        val allFormats = FormatRegistry.formats
        val ftpFormat = allFormats.find { it.id == FormatRegistry.ID_FTP }

        assertNotNull(ftpFormat)
        assertThat(ftpFormat.name).isEqualTo("FTP")
    }
}
