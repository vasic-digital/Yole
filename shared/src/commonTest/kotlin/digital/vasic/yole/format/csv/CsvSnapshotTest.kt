...
/**
 * Snapshot tests for CSV format components.
 *
 * Tests cover:
...
class CsvSnapshotTest {
    ...
    @Test
    fun `should match snapshot for CSV editor with basic content`() {
        ...
        val snapshot = captureSnapshot("csv_editor_basic")
        assertSnapshotMatches(snapshot, "csv_editor_basic")
    }

    @Test
    fun `should match snapshot for CSV editor with empty content`() {
        composeTestRule.setContent {
            CsvEditor(
                content = "",
                onContentChange = {},
        ...
        val snapshot = captureSnapshot("csv_editor_empty")
        assertSnapshotMatches(snapshot, "csv_editor_empty")
    }

    @Test
    fun `should match snapshot for CSV editor with long content`() {
        val longContent = """
...
Line 3
        ...
        val snapshot = captureSnapshot("csv_editor_long")
        assertSnapshotMatches(snapshot, "csv_editor_long")
    }

    // ==================== Preview Snapshot Tests ====================

    @Test
    fun `should match snapshot for CSV preview with light theme`() {
        composeTestRule.setContent {
            CsvPreview(
                content = "Sample CSV content here",
                isDarkTheme = false
        ...
        val snapshot = captureSnapshot("csv_preview_light")
        assertSnapshotMatches(snapshot, "csv_preview_light")
    }

    @Test
    fun `should match snapshot for CSV preview with dark theme`() {
        composeTestRule.setContent {
            CsvPreview(
                content = "Sample CSV content here",
                isDarkTheme = true
        ...
        val snapshot = captureSnapshot("csv_preview_dark")
        assertSnapshotMatches(snapshot, "csv_preview_dark")
    }

    @Test
    fun `should match snapshot for CSV preview with format-specific content`() {
        composeTestRule.setContent {
            CsvPreview(
        ...
        val snapshot = captureSnapshot("csv_preview_formatted")
        assertSnapshotMatches(snapshot, "csv_preview_formatted")
    }
    ...
    fun `should match snapshot for editor and preview side by side`() {
        ...
        val snapshot = captureSnapshot("csv_editor_preview_split")
        assertSnapshotMatches(snapshot, "csv_editor_preview_split")
    }

    @Test
    fun `should match snapshot for toolbar with CSV actions`() {
        composeTestRule.setContent {
            Surface {
                Row {
        ...
        val snapshot = captureSnapshot("csv_toolbar")
        assertSnapshotMatches(snapshot, "csv_toolbar")
    }
    ...
    fun `should match snapshot for mobile layout`() {
        ...
        val snapshot = captureSnapshot("csv_mobile_layout")
        assertSnapshotMatches(snapshot, "csv_mobile_layout")
    }
    ...
    fun `should match snapshot for tablet layout`() {
        ...
        val snapshot = captureSnapshot("csv_tablet_layout")
        assertSnapshotMatches(snapshot, "csv_tablet_layout")
    }
    ...
    fun `should match snapshot for desktop layout`() {
        ...
        val snapshot = captureSnapshot("csv_desktop_layout")
        assertSnapshotMatches(snapshot, "csv_desktop_layout")
    }
    ...
    fun `should match snapshot for error state`() {
        composeTestRule.setContent {
            Surface(color = Color.Red.copy(alpha = 0.1f)) {
                Text(
                    text = "Error: Invalid CSV content",
                    color = Color.Red,
        ...
        val snapshot = captureSnapshot("csv_error_state")
        assertSnapshotMatches(snapshot, "csv_error_state")
    }
    ...
    fun `should match snapshot for loading state`() {
        ...
        val snapshot = captureSnapshot("csv_loading_state")
        assertSnapshotMatches(snapshot, "csv_loading_state")
    }
    ...
    fun `should match snapshot for high contrast mode`() {
        ...
        val snapshot = captureSnapshot("csv_high_contrast")
        assertSnapshotMatches(snapshot, "csv_high_contrast")
    }
    ...
    fun `should match snapshot with large font size`() {
        ...
        val snapshot = captureSnapshot("csv_large_font")
        assertSnapshotMatches(snapshot, "csv_large_font")
    }
