 ...
 *
 * Integration tests for LaTeX format
 *
 *########################################################*/
...
class LatexIntegrationTest {
    ...
    fun `content detection should match extension detection`() {
        ...
        val byExtension = FormatRegistry.getByExtension(".tex")
    ...
    fun `should not conflict with other formats`() {
        ...
        // Ensure LaTeX format is unique
        val latexFormats = formats.filter { it.id == FormatRegistry.ID_LATEX }
        assertThat(latexFormats).hasSize(1)
    }
