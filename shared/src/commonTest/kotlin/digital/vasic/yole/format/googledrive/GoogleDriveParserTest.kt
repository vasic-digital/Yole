 ...
 *
 * Unit tests for GoogleDrive parser
 *
 *########################################################*/
...
/**
 * Unit tests for GoogleDrive format parser.
 *
 * Tests cover:
...
class GoogleDriveParserTest {
    ...
    @Test
    fun `should detect GoogleDrive format by extension`() {
        val format = FormatRegistry.getByExtension(".gdoc")

        assertNotNull(format)
        assertThat(format.id).isEqualTo(FormatRegistry.ID_GOOGLEDRIVE)
        assertThat(format.name).isEqualTo("GoogleDrive")
    }

    @Test
    fun `should detect GoogleDrive format by filename`() {
        val format = FormatRegistry.detectByFilename("test.gdoc")
    ...
    @Test
    fun `should support all GoogleDrive extensions`() {
        val extensions = listOf(".gdoc")
    ...
    @Test
    fun `should parse basic GoogleDrive content`() {
        val content = """
            Sample GoogleDrive content here
        """.trimIndent()
    ...
    fun `should handle single line input`() {
        val content = "Single line of GoogleDrive"
    ...
    fun `should not false-positive on plain text`() {
        ...
        // Should detect as plaintext, not GoogleDrive
        if (format != null) {
            assertThat(format.id).isNotEqualTo(FormatRegistry.ID_GOOGLEDRIVE)
        }
    }
    ...
    fun `should handle very long input`() {
        val longContent = "Single line of GoogleDrive\n".repeat(10000)
    ...
    fun `should integrate with FormatRegistry`() {
        val format = FormatRegistry.getById(FormatRegistry.ID_GOOGLEDRIVE)

        assertNotNull(format)
        assertThat(format.name).isEqualTo("GoogleDrive")
        assertThat(format.defaultExtension).isEqualTo(".gdoc")
    }

    @Test
    fun `should be registered in FormatRegistry`() {
        val allFormats = FormatRegistry.formats
        val googledriveFormat = allFormats.find { it.id == FormatRegistry.ID_GOOGLEDRIVE }

        assertNotNull(googledriveFormat)
        assertThat(googledriveFormat.name).isEqualTo("GoogleDrive")
    }
}
