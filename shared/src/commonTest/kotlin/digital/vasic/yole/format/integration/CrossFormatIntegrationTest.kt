...
class CrossFormatIntegrationTest {
    ...
    fun `test performance of cross-format operations`() {
        ...
        assertTrue(parseTime.inWholeMilliseconds < 1000, "Parsing should complete within 1 second")
        assertTrue(htmlConversionTime.inWholeMilliseconds < 500, "HTML conversion should complete within 500ms")
    }
    ...
    fun `test comprehensive integration benchmarks`() {
        ...
        benchmarkResults.values.forEach { duration ->
            assertTrue(duration.inWholeMilliseconds > 0, "Benchmark should take measurable time")
        }
    }
    ...
    private fun convertJupyterToRMarkdown(jupyterContent: String): String {
        ...
        return buildString {
            ...
            appendLine("---")
            appendLine()
            appendLine("# Converted from Jupyter")
            appendLine()
            appendLine("This document was converted from a Jupyter notebook.")
            appendLine()
            
            // Extract some content from the JSON
            if (jupyterContent.contains("Data Analysis")) {
                appendLine("## Data Analysis")
                appendLine("Analysis content from the original notebook.")
            }
            if (jupyterContent.contains("Machine Learning")) {
                appendLine("## Machine Learning")
                appendLine("ML content from the original notebook.")
            }
        }
    }
