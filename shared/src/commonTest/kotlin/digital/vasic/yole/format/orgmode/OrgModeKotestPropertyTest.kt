 ...
 *
 * Property-based tests using Kotest for Org Mode format
 *
 *########################################################*/
...
/**
 * Property-based tests for Org Mode format using Kotest.
 *
 ...
 */
class OrgModePropertyTest : StringSpec({

    val parser = OrgModeParser()

    // ==================== Basic Property Tests ====================

    "parsing any valid Org Mode content should not throw exceptions" {
        checkAll(validOrgModeContent()) { content ->
    ...
    "parsing then serializing should preserve content structure" {
        checkAll(validOrgModeContent()) { original ->
    ...
    "parser should handle different line ending styles" {
        checkAll(validOrgModeContent(), Arb.lineEnding()) { content, lineEnding ->
    ...
    "should maintain format-specific property 1" {
        checkAll(formatSpecificOrgModeContent()) { input ->
    ...
    "parser should gracefully handle malformed input" {
        checkAll(malformedOrgModeContent()) { malformed ->
    ...
    "parser should handle truncated content" {
        checkAll(validOrgModeContent(), Arb.int(0..100)) { content, cutoff ->
    ...
    "parsing large content should complete in reasonable time" {
        checkAll(10, largeValidOrgModeContent()) { largeContent ->
...
/**
 * Generates valid Org Mode content samples.
 */
fun validOrgModeContent(): Arb<String> = arbitrary { rs ->
    val samples = listOf(
        "Sample Org Mode content here",
        "Single line of Org Mode",
        "Line 1
Line 2
Line 3",
        ""
...
/**
 * Generates large valid Org Mode content for performance testing.
 */
fun largeValidOrgModeContent(): Arb<String> = arbitrary { rs ->
    buildString {
        repeat(rs.random.nextInt(500, 2000)) {
            appendLine("Single line of Org Mode")
        }
    }
}
...
/**
 * Generates malformed Org Mode content for error handling tests.
 */
fun malformedOrgModeContent(): Arb<String> = arbitrary { rs ->
 ...
 */
fun formatSpecificOrgModeContent(): Arb<String> = arbitrary { rs ->
