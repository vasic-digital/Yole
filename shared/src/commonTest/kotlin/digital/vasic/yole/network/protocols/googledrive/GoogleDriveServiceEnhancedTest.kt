...
class GoogleDriveServiceEnhancedTest {
    ...
    fun testEnhancedStorageInfo() = runTest {
        ...
        assertEquals("onedrive://", storageInfo.location) // Note: This might be a bug in the original code
    ...
    fun testEnhancedSyncOperations() = runTest {
        ...
        assertTrue(syncAllOps.isEmpty(), "Sync all should return empty when not connected")
    }
    ...
    fun testEnhancedConfigurationValidation() {
        ...
        assertEquals("root", minimalService.config.rootFolderId) // Should default to "root"
