...
class NetworkStorageIntegrationTest {
    ...
    fun testUnifiedFileOperations() = runTest {
        ...
        services.forEach { (service, name) ->
            ...
            assertTrue(quotaResult.isSuccess, "$name getQuotaInfo should succeed")
            val quota = quotaResult.getOrNull()
            assertNotNull(quota, "$name should return quota info")
    ...
    fun testCrossProtocolCompatibility() = runTest {
        ...
        results.forEach { (isOnline, quotaSuccess, existsResult) ->
            // Basic operations should complete (results may vary based on connection state)
            assertTrue(quotaSuccess, "All services should handle quota requests")
